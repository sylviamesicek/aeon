...
pub fn evolve_data_full(
    ...
    let mut fixed_grid_begun = false;
    let mut fixed_grid_ended = false;
    ...
    while proper_time < config.evolve.max_proper_time {
        ...
        // Perform regridding if necessary
        if steps_since_regrid > config.regrid.flag_interval {
            ...
            // Perform a step towards fixed refinement in the center, if necessary
            if config.regrid.fix_grid && proper_time >= config.regrid.fix_grid_time && !fixed_grid_ended {
                fixed_grid_begun = true;
