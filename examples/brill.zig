...
pub const BrillInitialData = struct {
    const N = 2;
    const M = 3;
    const O = 2;
    ...
    fn run(allocator: Allocator) !void {
        ...
        worker.order(O).projectAll(Seed{ .amplitude = 1.0, .sigma = 1.0 }, seed);
        worker.order(O).fillGhostNodesAll(Boundary{}, seed);
...
pub fn main() !void {
    ...
    // Run main
    try BrillInitialData.run(gpa.allocator());
}
