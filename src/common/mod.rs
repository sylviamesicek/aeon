...
pub trait Convolution<const N: usize> {
    ...
    fn negative(self: &Self, position: [f64; N], spacing: f64) -> Self::NegativeBoundary;
    fn positive(self: &Self, position: [f64; N], spacing: f64) -> Self::PositiveBoundary;
}
...
impl<'a, const N: usize> NodeSpaceAxis<'a, N> {
    ...
    pub fn evaluate<K: Convolution<N>>(
        ...
        for mut node in self.space.vertex_space().plane(self.axis, 0) {
            ...
            let negative_boundary = convolution.negative(negative_position, spacing);
            ...
            let positive_boundary = convolution.positive(positive_position, spacing);
    ...
    fn negative_ghost_value<B: Boundary>(
        ...
        for (i, w) in StencilIterator::new(boundary.stencil(extent)).enumerate() {
            node[self.axis] = i;
