...
pub trait Conditions<const N: usize>: Clone {
    ...
    fn parity(&self, _field: Self::Label, _face: Face<N>) -> bool {
        false
    }
...
impl<const N: usize, I: Condition<N>> Conditions<N> for ScalarConditions<I> {
    ...
    fn parity(&self, _field: Self::Label, face: Face<N>) -> bool {
        self.0.parity(face)
    }

    fn radiative(&self, _field: Self::Label, position: [f64; N], spacing: f64) -> RadiativeParams {
...
impl<const N: usize, S: Clone, C: Conditions<N, Label = S>> Condition<N> for SystemCondition<S, C> {
...
#[derive(Clone)]
pub struct SystemBC<S, B, C> {
    field: S,
    boundary: B,
    conditions: C,
}

impl<S, B, C> SystemBC<S, B, C> {
...
impl<const N: usize, S: Clone, B: Clone, C: Conditions<N, Label = S>> Condition<N>
    for SystemBC<S, B, C>
{
    fn parity(&self, face: Face<N>) -> bool {
        self.conditions.parity(self.field.clone(), face)
...
impl<const N: usize, S: Clone, B: Boundary<N>, C: Clone> Boundary<N> for SystemBC<S, B, C> {
    fn kind(&self, face: Face<N>) -> BoundaryKind {
...
impl<const N: usize, L: Conditions<N>, R: Conditions<N>> Conditions<N> for PairConditions<L, R> {
    type Label = Pair<L::Label, R::Label>;

    fn parity(&self, field: Self::Label, face: Face<N>) -> bool {
        match field {
    ...
    fn radiative(&self, field: Self::Label, position: [f64; N], spacing: f64) -> RadiativeParams {
        match field {
