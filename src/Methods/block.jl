...
function Bases.blockcells(block::TreeBlock{N, T}) where {N, T}
    refinement::Int = block.surface.tree.refinement
    ntuple(i -> 2^refinement, Val(N))
end

Bases.blockbounds(block::TreeBlock) = block.surface.tree.bounds[block.node]
...
function Bases.value(field::TreeField{N, T}, block::TreeBlock{N, T}, cell::CartesianIndex{N, T}) where {N, T}
    # Find offset 
...
function Bases.setvalue!(field::TreeField{N, T}, value::T, block::TreeBlock{N, T}, cell::CartesianIndex{N, T}) where {N, T}
    # Find offset 
    offset = block.surface.offsets[block.node]
    # Build map to linear
