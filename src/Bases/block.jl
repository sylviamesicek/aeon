...
export Domain, domaincells, domainbounds, domaintransform
export cellindices, cellwidths, cellcenter
export Field, Block, value, setvalue!, prolong
...
function _prolong_product(field::Field{N, T}, block::Block{N, T}, basis::Basis{T}, ::Val{O}, cell::NTuple{N, CellIndex}) where {N, T, L, M, O}
    ...
    quote
        ...
        if leftcells < $O
            Base.@nexprs $O i -> begin
                if leftcells == i - 1
                    return _prolong_vertex(field, block, vertex_value_stencil(basis, Val(i - 1), Val(2O + 1)), basis, Val($O), cell, index, rest...)
                end
            end
        end
        ...
        if rightcells < $O
            Base.@nexprs $O i -> begin
                if rightcells == i - 1
                    return _prolong_vertex(field, block, vertex_value_stencil(basis, Val(2O + 1), Val(i - 1)), basis, Val($O), cell, index, rest...)
                end
            end
        end
...
value(domain::Domain{N, T}, cell::CartesianIndex{N, T}) where {N, T} = domain.inner[CartesianIndex(cell.I .+ O)]
...
function domainprolong(domain::Domain{N, T, O}, cell::NTuple{N, CellIndex}, ::Basis{T}) where {N, T, O}
    value(domain, CartesianIndex(map(i -> i.inner, cell)))
end
...
function transfer_block_to_domain!(domain::Domain{N, T}, field::Field{N, T}, block::Block{N, T}) where {N, T}
    # Meta data
    ...
    for cell in cellindices(block)
        ...
        setvalue!(domain, v, cell)
    end

    # Fill exterior values
    fill_interfaces!(domain, field, block)
end

@generated function fill_interfaces!(domain::Domain{N, T, O}, field::Field{N, T}, block::Block{N, T}) where {N, T, O}
    Base.@nexprs O i -> begin
