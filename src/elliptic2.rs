        ...
        for index in 0..self.max_steps {
            {
                ...
                systems.save_system_default(SystemSlice::from_contiguous(
                    &data,
                    &(result.system().clone(), result.system().clone()),
                ));
    ...
    C::System: Sync,
{
    ...
    fn derivative(&mut self, f: &mut [f64]) {
        self.tmp.clear();
        self.tmp.resize(f.len(), 0.0);

        let (udata, vdata) = f.split_at_mut(self.dimension);
        let mut u = SystemSliceMut::from_contiguous(udata, self.system);
        let mut v = SystemSliceMut::from_contiguous(vdata, self.system);
        ...
        let (udata, vdata) = self.tmp.split_at_mut(self.dimension);
