...
/// A class for handling permutations from uniform cells in index space to quadtree space.
/// For instance, the permutation at level = 2 is equal to:
///
///  y
///  .
/// /|\
...
///  --------------------> x
///
/// To simplify various operations and algorithms on meshes, we ensure that all cells of a given level are stored
/// contiguously.
pub fn CellPermutation(comptime N: usize) type {
    return struct {
        ...
        /// Builds a new permutation map, sufficient up to the maximum refinement.
        pub fn init(allocator: Allocator, levels: usize) !@This() {
            // Compute offsets
            var offsets: []usize = try allocator.alloc(usize, levels + 1);
            ...
            // Compute buffer
            var buffer: []usize = try allocator.alloc(usize, offsets[levels]);
        ...
        pub fn permutation(self: @This(), level: usize) []const usize {
            return self.buffer[self.offsets[level]..self.offsets[level + 1]];
        }

        pub fn numLevels(self: @This()) usize {
...
test "cell permutation" {
    ...
    try expect(permute.numLevels() == 4);
    try expect(permute.offsets[0] == 0);
    try expect(permute.offsets[1] == 1);
    try expect(permute.offsets[2] == 5);
    try expect(permute.offsets[3] == 21);
    try expect(permute.offsets[4] == 85);
