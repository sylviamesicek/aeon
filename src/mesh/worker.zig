...
pub fn NodeWorker(comptime N: usize, comptime M: usize) type {
    return struct {
        ...
        fn prolongBlock(self: @This(), comptime O: usize, block_id: usize, field: []f64) void {
            ...
            const block_field = self.manager.blockNodes(block_id, field);
        ...
        fn restrictBlock(self: @This(), comptime O: usize, block_id: usize, field: []f64) void {
            ...
            while (indices.next()) |offset| {
                ...
                const val = block_node_space.restrictFull(O, toSigned(offset), block_field);
        ...
        pub fn dissipation(self: @This(), comptime O: usize, eps: f64, deriv: []f64, src: []const f64) void {
            assert(deriv.len == self.manager.numNodes());
            assert(src.len == self.manager.numNodes());
        ...
        fn dissipationBlock(self: @This(), comptime O: usize, block_id: usize, eps: f64, deriv: []f64, src: []const f64) void {
            const block_deriv: []f64 = self.manager.blockNodes(block_id, deriv);
            const block_src: []const f64 = self.manager.blockNodes(block_id, src);
            const block_bounds = self.manager.blocks.items(.bounds)[block_id];
            const block_size = self.manager.blocks.items(.size)[block_id];

            var spacing: [N]f64 = undefined;

            for (0..N) |axis| {
                spacing[axis] = block_bounds.size[axis];
                spacing[axis] /= @floatFromInt(block_size[axis] * self.manager.cell_size[axis]);
            }
        ...
        pub fn fillGhostNodes(self: @This(), comptime O: usize, set: anytype, field: []f64) void {
            ...
            for (1..self.manager.numBlocks()) |block_id| {
        ...
        fn fillBaseGhostNodes(self: @This(), comptime O: usize, set: anytype, field: []f64) void {
            ...
            const boundary_engine = BoundaryEngine(N, M, @TypeOf(set)){
                .space = node_space,
                .range = self.manager.block_to_nodes.range(0),
                .set = set,
            };
        ...
        fn fillBlockGhostNodes(self: @This(), comptime O: usize, block_id: usize, set: anytype, field: []f64) void {
            ...
            const block_boundary = self.manager.blocks.items(.boundary)[block_id];
            const node_space = self.blockNodeSpace(block_id);

            const boundary_engine = BoundaryEngine(N, M, @TypeOf(set)){
                .space = node_space,
                .range = self.manager.block_to_nodes.range(block_id),
                .set = set,
            };
        ...
        fn fillBlockGhostNodesInterior(self: @This(), comptime O: usize, region: Region, block_id: usize, field: []f64) void {
            ...
            while (cell_indices.next()) |index| {
                ...
                const origin = toSigned(self.manager.cellNodeOrigin(cell));
                ...
                if (neighbor == null_index) {
                    ...
                    const neighbor_field: []const f64 = self.manager.blockNodes(neighbor_block_id, field);
                    ...
                    const neighbor_block_id = cell_meta.items(.block)[neighbor];
                    const neighbor_index = cell_meta.items(.index)[neighbor];
                    const neighbor_field = self.manager.blockNodes(neighbor_block_id, field);
                    const neighbor_node_space = self.blockNodeSpace(neighbor_block_id);
